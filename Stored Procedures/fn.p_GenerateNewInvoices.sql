...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [fn].[p_GenerateNewInvoices]

as
/***
	Author: Mike Sherman
	Date:   
	Desc:  generates new set of invoices. Need logic to prevent repeating same invoices

	2014-05-21	msh added four fields to the final update to deal with special arrangements
	select * from fn.invoices_stage order by 1 desc
		select * from fn.invoices order by 1 desc
...
-- in June we send invoices only to students with overdue amount from previous year. We simply delete all records with past due = 0 from staging table
-- per Bill we should send a reminder in June as well.
	--if month (getdate()) in (6)
	--	delete from fn.Invoices_Stage
