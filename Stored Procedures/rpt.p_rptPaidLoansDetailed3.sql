...
SET ANSI_NULLS ON
GO
CREATE PROC [rpt].[p_rptPaidLoansDetailed3]
	(@FromDate date
	,@ToDate date
	,@LoanDate date)
AS 
...
--kk Converted datetime to date on February 14, 2022
					...
					,'Loan 1 Status' = CASE WHEN fn.Loans.LoanSeqNum = 1 THEN fn.Loans.LoanStatus END
...
--kk Converted datetime to date on February 14, 2022
					...
					,'Loan 2 Status' = CASE WHEN fn.Loans.LoanSeqNum = 2 THEN fn.Loans.LoanStatus END
					--Date on which the loan was paid off
					--kk Converted datetime to date on February 14, 2022
					,'Date Paid' = CONVERT(date, #LastPayment.LastPaymentDate)
					...
					,'Paid Off In # Years' = #DateDifferenceYear.NoOfYears 
		...
		FROM #PaidOffLoans 
		INNER JOIN fn.Loans					ON #PaidOffLoans.LoanID = fn.Loans.LoanID 
		...
		INNER JOIN #LastPayment				ON #PaidOffLoans.LoanID = #LastPayment.LoanID
		LEFT JOIN #PrincipalBalance			ON #PaidOffLoans.LoanID = #PrincipalBalance.LoanID
		LEFT JOIN #Interest					ON #PaidOffLoans.LoanID = #Interest.LoanID
		INNER JOIN #InterestPaid			ON #PaidOffLoans.LoanID = #InterestPaid.LoanID
		INNER JOIN #LateCharges				ON #PaidOffLoans.LoanID = #LateCharges.LoanID
		INNER JOIN #DateDifference			ON #PaidOffLoans.LoanID = #DateDifference.LoanID
		INNER JOIN #DateDifferenceYear		ON #PaidOffLoans.LoanID = #DateDifferenceYear.LoanID
		LEFT JOIN #LastInterest				ON #PaidOffLoans.LoanID = #LastInterest.LoanID
		INNER JOIN #GetLoanDate				ON #PaidOffLoans.LoanID = #GetLoanDate.LoanID
