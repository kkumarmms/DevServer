...
--exec rpt.p_rptMonthlyMasterDEBUG 
...
CREATE PROCEDURE [rpt].[p_rptMonthlyMasterDEBUG] 
AS
BEGIN
-- =============================================
-- Author:		Andre Barber
...
Insert into #Loan1			
	...
	, lps.[PrincipalAmtDue] as 'Principal1' ,lps.[InterestAmtDue] as 'Interest1'
	 FROM  
			...
			fn.Loans.UserID 	in (904,1819,705,2693) and
...
;
	WITH data1PivotPaid (UserID,LastName,FirstName,LoanItemDescr,PaymentDate,PaymentCode,ItemAmtPaid ) as
				(
				...
				and	fn.Loans.UserID 	in (904,1819,705,2693) 
...
select UserID,SUM([Principal Due]),SUM([Principal Overdue]) from #Loan1PivotPaid --order by 		UserID				
		group by UserID			
			;

		WITH data1PivotCurrent (UserID,LastName,FirstName,LoanItemDescr,ItemAmt) as
				(
...
select * from #Loan1PivotCurrent order by 		UserID				
	...
	WITH data2PivotPaid (UserID,LastName,FirstName,LoanItemDescr,PaymentDate,PaymentCode,ItemAmtPaid ) as
				(
				SELECT     act.UserInfo.UserID, act.UserInfo.LastName,act.UserInfo.FirstName, fn.LoanItems.LoanItemDescr, fn.LoanPayment.PaymentDate, fn.LoanPayment.PaymentCode, ItemAmtPaid =sum(fn.LoanPaymentApply.AppliedAmt) 
				FROM         act.UserInfo INNER JOIN
                      fn.LoanPaymentApply ON act.UserInfo.UserID = fn.LoanPaymentApply.UserID LEFT OUTER JOIN
...
select UserID,SUM([Principal Due]),SUM([Principal Overdue]) from #Loan2PivotPaid --order by 		UserID				
		group by UserID
			;

		WITH data2PivotCurrent (UserID,LastName,FirstName,LoanItemDescr,ItemAmt) as
				(
				SELECT     act.UserInfo.UserID, act.UserInfo.LastName,act.UserInfo.FirstName, fn.LoanItems.LoanItemDescr, ItemAmt =sum(fn.LoanCurrState.LoanItemAmt) 
				FROM         act.UserInfo INNER JOIN
									  fn.LoanCurrState ON act.UserInfo.UserID = fn.LoanCurrState.UserId INNER JOIN
...
group by UserID
	...
	select '#LoanPivotPaidTotals this is ok'
	select * from #LoanPivotPaidTotals --this is ok
...
,sum(#LoanPivotPaidTotals.Principal) as 'Principal Paid to Date'
...
,sum(#LoanPivotPaidTotals.Interest) as 'Interest Paid to Date' 
...
group by #Output.Acct,#Output.Name,#Output.[Date Of Loan1],#Output.[Date Of Loan2],#Output.[Original Principal],#Output.[Original Interest]
