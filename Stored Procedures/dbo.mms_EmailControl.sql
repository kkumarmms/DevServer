...
CREATE PROCEDURE [dbo].[mms_EmailControl]
as
/*
...
----update mms_email set recipients = 'msherman@mms.org;rdatla@mms.org' where MailSent is null

-- only for initial go-live period
update mms_email set BCC_Recipients = 'msherman@mms.org;svatasoiu@mms.org;oteixeira@mms.org' where MailSent is null
...
declare	
	...
	begin
		...
		select 	
			...
			@Subject			=Subject + ' for - ' + recipients ,
			@body				=body ,
...
ALTER ROLE [db_owner] ADD MEMBER [dom1\svatasoiu]
GO
