...
CREATE PROCEDURE [opr].[p_GetPaymentSchedule] --@UserId=2522, @CurrDate='2007-10-31',@PrincipalBalance=4000
	@UserId int,
	@CurrDate date = null,
...
/***
	...
	Desc:  Calculates payment schedule for the student based on the current Amount owed
		...
		2021-05-06 - msh Changed section for May-no Overdue. Payment due date should be 06/30 not @lastDayOfMonth
[opr].[p_GetPaymentSchedule] @UserId=3254, @CurrDate='2021-02-25',@PrincipalBalance=3662
select * from [fn].[LoanCurrState] where   UserId=3310
***/
...
--**************Changed to split DUE and OVERDUE amounts for the first year**************
	...
	--select 'c'
		...
		--select 'd'
			...
			UNION
			select  -- new amount for the current year
					ID,
		...
		ELSE -- no total overdue May only
		BEGIN
		--select 'e'
				select  ID,
			PrincipalBalance= @CurrBalance, 
